...
const token = localStorage.getItem('token')
  const {data:{defaultStats, monthlyApplications},} = await axios.get(`/api/v1/tickets/allStats`,
  {
      headers: {
...
.map((ticket) => {
var {subject, department, location, category, subCategory, approval, status, createdAt, createdBy,approvedBy, assignedTo, remark, _id:ticketID} = ticket
...
const allTickets = ticketsFilter
.map((ticket) => {
var {subject, department, location, category, subCategory, approval, status, createdBy,approvedBy, assignedTo, createdAt, remark, _id:ticketID} = ticket
var date = createdAt.slice(0, 10)
date = date.split('-').reverse().join('/')
