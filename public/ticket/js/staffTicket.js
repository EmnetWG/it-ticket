...
const {
    data:{tickets},
} = await axios.get(`/api/v1/tickets/assignedTickets`, {
    headers: {
      Authorization: `Bearer ${token}`,
    }},)
...
.map((ticket) => {
var {subject, department, location, category, subCategory, approval, status, createdBy,approvedBy, assignedTo, remark, createdAt, _id:ticketID} = ticket
...
const {
    ...
    const {subject, department, location, category, subCategory, approval, 
      status, createdBy, approvedBy, assignedTo, remark, createdAt, _id:ticketID} = ticket.ticket
